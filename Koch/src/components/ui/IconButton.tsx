...
export function IconButton({
    ...
    const baseBtnCls = [
        "relative text-foreground-dark/90 transition-all duration-150 ease-out",
        variant === "rounded"
            ? "rounded-full p-2 border border-transparent"
            : "p-2 rounded-md border-[1px] border-primary-dark/20",
        variant === "rounded"
            ? active && !disabled
                ? "bg-primary/70"
                : "bg-card-dark"
    ...
    const tooltipBg = disabled
        ? "bg-primary-dark/30 text-foreground-dark/70"
        : "bg-primary-dark text-foreground-dark";
        ...
        <div className="relative flex flex-col items-center" aria-disabled={disabled}>
            ...
            {tooltip && (
                ...
                >
                    ...
                    <div
                        className={`absolute left-1/2 ${arrowCls} -translate-x-1/2 w-2 h-2 rotate-45 z-10 ${disabled ? "bg-primary-dark/30" : "bg-primary-dark"}`}
                    />
                </div>
            )}
        </div>
