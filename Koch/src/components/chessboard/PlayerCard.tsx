...
export function PlayerCard({ display, player, color, rating: _rating, clock, isTurn }: Props) {
        ...
        <div className="flex flex-col items-end ml-3">
          {typeof clock !== "undefined" && (
            <div className={`flex items-center gap-2 ${isTurn ? "text-foreground-dark" : "text-foreground-dark/40"}`}>
              ...
              <span className="font-mono text-xl  tracking-widest">{clock.toString().slice(0, -4)}</span>
            </div>
          )}
        </div>
      </div>
