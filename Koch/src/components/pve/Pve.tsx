...
export function Pve() {
  ...
  const [_gameMode, setGameMode] = useState<GameControllerMode>("Rapid")
  const [blackTaken, _setBlackTaken] = useState<[PieceType, PieceColor][]>([]);
  const [whiteTaken, _setWhiteTaken] = useState<[PieceType, PieceColor][]>([]);
  ...
  if (chessBoard === null) {
    return (
      <div>
        <h1 className="text-2xl">Could not get board from backend</h1>
      </div>
    )
  }
        ...
        <div className="flex flex-row w-[100%] h-[100%]">
          <div className="relative w-full h-full flex flex-col justify-center items-center w-[75%]">
            ...
            <div className="relative w-auto h-auto">
                    ...
                    >
                      <div className="bg-card-dark px-12 flex flex-col items-center justify-center rounded-lg shadow-lg p-6 gap-4">
                        ...
                        <div className="flex flex-row gap-4 mb-2">
                          <span className="text-base text-white">{chessGame.player_elo}</span>
                        ...
                        <button className="px-4 py-2 bg-accent text-white rounded hover:bg-accent/80 transition"
                          onClick={async () => {
                            await invoke<string | null>('save_appgame');
              ...
              {(() => {
                const isFlipped = chessGame ? chessGame.player === "Black" : false;
                const topColor = isFlipped ? "White" : "Black";
                const bottomColor = isFlipped ? "Black" : "White";
                const topPlayer = topColor === "White" ? chessGame?.player_card : chessGame?.engine_card;
                const bottomPlayer = bottomColor === "White" ? chessGame?.player_card : chessGame?.engine_card;
                const topClock = topColor === "White" ? formatClock(whitePlayerClock) : formatClock(blackPlayerClock);
                const bottomClock = bottomColor === "White" ? formatClock(whitePlayerClock) : formatClock(blackPlayerClock);
                return (
                  <div className="flex flex-col" style={{ width: squareSize * 8 }}>
                    <PlayerCard
                      display={true}
                      color={topColor as "White" | "Black"}
                      player={topPlayer ?? ""}
                      clock={topClock}
                      isTurn={chessBoard?.turn === topColor}
                    />
                    <ChessBoard
                      board={chessBoard}
                    ...
                    <PlayerCard
                      ...
                      player={bottomPlayer ?? ""}
                      clock={bottomClock}
                      isTurn={chessBoard?.turn === bottomColor}
                    />
                  </div>
                );
              })()}
            ...
            <div className="flex flex-row gap-4 items-center mt-4">
              ...
              <IconButton
                ...
                icon={chessGame?.can_be_abandoned ? <Ban className="text-destructive-dark" size={iconSize} /> : <Flag className="text-destructive-dark" size={iconSize} />}
                tooltip={chessGame?.can_be_abandoned ? 'Abbandon' : 'Surrender'}
                disabled={chessGame?.state === "AwaitingStart"}
              />
              <IconButton
                onClick={async () => {
                ...
                icon={chessGame?.state === "Ended" ? <RotateCcw size={iconSize} /> : <Play size={iconSize} />}
                tooltip="Start game"
              ...
              <button onClick={async () => {
                ...
                <IconButton icon={<Share2 size={iconSize} />} tooltip="Share game" />
              </button>
            </div>
          </div>
