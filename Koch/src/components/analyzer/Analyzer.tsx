...
import { Download, FileText, FlipVertical, MessageSquareText } from "lucide-react";
...
import { ChessBoard, GhostPiece } from "../chessboard/Chessboard";
...
export function Analyzer({ gameId, setSelectedGameId }: AnalyzerProps) {
  ...
  const [barEval, _setBarEval] = useState<number>(0.0);
  ...
  const [_whiteClock, setWhiteClock] = useState<string | null>(null);
  const [_blackClock, setBlackClock] = useState<string | null>(null);
  ...
  const [boardPov, setBoardPov] = useState<PieceColor>("White");
  ...
  /** Parse the first UCI-like token (e.g. "e2e4") from a move string and return an ArrowData, or null if invalid. */
  const parseUciArrow = (moveString: string, color: string): ArrowData | null => {
    const firstToken = moveString.trim().split(/\s+/)[0];
    if (!firstToken || firstToken.length < 4) return null;
    const fromSq = firstToken.slice(0, 2);
    const toSq = firstToken.slice(2, 4);
    const sqRe = /^[a-h][1-8]$/;
    if (!sqRe.test(fromSq) || !sqRe.test(toSq)) return null;
    try {
      const from = fileRankToRowCol(fromSq);
      const to = fileRankToRowCol(toSq);
      return { from: [from[0], from[1]] as [number, number], to: [to[0], to[1]] as [number, number], color, type: "engine" as const };
    } catch {
      return null;
    }
  };
  ...
  const updateSuggestion = (pv: PvObject | null) => {
    try {
      ...
      setSuggestion(parseUciArrow(bestLine.moves, "blue"));
    } catch (err) {
  ...
  const updateThreat = (raw: string | null) => {
    ...
    setThreat(parseUciArrow(raw, "red"));
  }
    ...
    <div className="main-section w-full h-full flex flex-row flex-1 overflow-hidden">
      ...
      <div className="w-full flex flex-col relative">
        ...
        <div className="board-section flex flex-1 justify-center items-center">
          ...
          <div className="relative">
            {displayBoard && ( // RENDER FROM displayBoard
              <ChessBoard
                ...
                board={displayBoard}
                flipped={boardPov == "White" ? true : false}
          ...
          <div className="relative flex flex-col gap-4" style={{ height: squareSize * 9.5 }}>

            <button
              type="button"
              className={[
                "mx-4 w-fit text-foreground-dark/90 bg-card-dark p-2 rounded-md border-[1px] border-primary-dark/20 transition-all duration-150 ease-out",
                "hover:scale-105",
                "hover:bg-primary/70"
              ].join(" ")}
              onClick={() => setBoardPov((prev) => (prev === "White" ? "Black" : "White"))}
              title="Flip board"
            >
              <FlipVertical className="h-5 w-5 mx-auto" />
            </button>
            <AnalyzerInputs
              height={squareSize * 8}
